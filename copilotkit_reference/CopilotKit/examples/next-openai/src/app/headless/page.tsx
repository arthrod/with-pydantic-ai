...
/**
 * Render a full-screen modal that displays the provided message as pretty-printed JSON when open.
 *
 * @param message - The message data to display; it will be JSON.stringify'd with indentation for readability.
...
/**
 * Renders a full-page CopilotKit chat panel configured from URL query parameters.
 *
 * The component reads `serviceAdapter`, `runtimeUrl`, and `publicApiKey` from the current
 * URL's search parameters and passes them to CopilotKit. It mounts CopilotKit with the
 * embedded ChatApp inside a full-height white background.
 *
 * @returns The React element for the page: a full-screen container with CopilotKit and ChatApp,
 * configured using the `serviceAdapter`, `runtimeUrl`, and optional `publicApiKey` query parameters.
 */
...
/**
 * Renders a "scroll to bottom" button when the sticky container is not at the bottom.
 *
 * The button calls the stick-to-bottom context's `scrollToBottom` handler when clicked.
 *
 * @returns The button element that triggers scrolling to the bottom, or `null` when already at the bottom.
 */
...
/**
 * Renders an interactive chat application UI connected to CopilotKit.
 *
 * This component displays conversation messages, suggestion chips, an auto-resizing input, per-message actions (copy, details, feedback), integration points for copilot actions and interrupts, and a modal for viewing message details.
 *
 * @returns A React element that mounts the full-featured Copilot chat interface.
 */
function ChatApp() {
  const {
    visibleMessages,
