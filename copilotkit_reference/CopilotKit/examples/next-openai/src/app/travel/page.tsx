...
/**
 * Render the PanelPage that configures CopilotKit from URL search parameters and nests the TravelPlanner.
 *
 * Reads `serviceAdapter`, `runtimeUrl`, and `publicApiKey` from the current URL query string, derives CopilotKit props
 * (`runtimeUrl` and optional `publicApiKey`), and renders a CopilotKit wrapper containing the TravelPlanner component.
 *
 * @returns A React element that renders CopilotKit with the configured runtime and optional public API key, containing TravelPlanner
 */
...
/**
 * Render a travel planner UI that hosts a Copilot chat and a bookings panel.
 *
 * Registers two Copilot actions:
 * - `presentHotelOrLocation`: presents a hotel/location card (title, description, optional image).
 * - `bookHotel`: adds a booking to local state using `name`, `arrivalDate`, and `departureDate`.
 *
 * The bookings panel displays current bookings and allows removing individual items.
 *
 * @returns The travel planner UI as a JSX element containing the chat and bookings panel.
 */
