...
/**
 * Render a demo page that wraps the TextAreas example with CopilotKit configured from URL search parameters.
 *
 * The component reads `serviceAdapter`, `runtimeUrl`, and `publicApiKey` from the current URL search params and forwards them to CopilotKit.
 *
 * @returns A JSX element containing a CopilotKit wrapper configured from URL search params and the TextAreas example as its child.
 */
...
/**
 * Render interactive text areas demonstrating CopilotTextarea integration with persisted state and readable context registration.
 *
 * The component persists `textareaPurpose`, `copilotText`, and `detailsText` to local storage, exposes `detailsText` to Copilot features under the "sales_reply" category, registers a sample document as readable for that category, and provides a focus control for the CopilotTextarea. The CopilotTextarea is configured with autosuggestions (short suggestion tokens, stop characters, and debounce).
 *
 * @returns A React element containing: a purpose input, a CopilotTextarea with autosuggestions, a details textarea, and a button that focuses the CopilotTextarea.
 */
function TextAreas() {
