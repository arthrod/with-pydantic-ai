...
/**
 * Decides whether a GraphQL error should be presented to the user as a UI banner.
 *
 * Evaluates the error's extensions, known discovery/configuration error codes, explicit visibility flags,
 * common authentication/authorization message indicators, and legacy original-error stack traces to determine banner visibility.
 *
 * @param gqlError - The GraphQLError to evaluate for banner display.
 * @returns `true` if the error should be shown as a banner, `false` otherwise.
 */
...
/**
 * Accesses the current MessagesTap from React context.
 *
 * @returns The `MessagesTap` instance used to read or update the tapped messages.
 * @throws If called outside of a `<MessagesTapProvider>` (when no context is available).
 */
...
/**
 * Provides a MessagesTap context to descendants, exposing a lightweight snapshot API for reading and updating a message tap.
 *
 * @param children - React nodes that will receive the MessagesTap context
 */
...
/**
 * Provides Copilot messages state and error-handling to descendant components.
 *
 * Supplies the CopilotMessagesContext with `messages`, `setMessages`, `suggestions`, and
 * `setSuggestions`. Loads and updates messages for the active thread/agent, synchronizes a
 * lightweight MessagesTap snapshot, and routes GraphQL/runtime errors to UI banners and the
 * optional tracing `onError` handler when available.
 */
