...
/**
 * Wraps children with Copilot context providers, an error boundary, and an optional dev toast/usage banner.
 *
 * @param children - React nodes to be rendered inside the Copilot providers.
 * @param props - Props forwarded to the internal CopilotKit implementation.
 * @returns A React element that provides Copilot context and error handling around `children`.
 */
...
/**
 * Internal Copilot provider that builds and supplies the full Copilot runtime context and UI wrappers for a subtree.
 *
 * @param cpkProps - CopilotKitProps used to configure the runtime client, context, agents, and UI behavior; required fields are validated.
 * @returns A React element that provides the Copilot context and renders the configured children with messaging and optional usage banner. 
 */
...
/**
 * Creates a FunctionCallHandler that invokes frontend actions by function name.
 *
 * @param actions - Array of frontend actions that may be invoked by the returned handler.
 * @returns The handler function which, when called with `{ name, args }`, invokes the matching action's `handler` and resolves to its result; resolves to `undefined` if no matching action is found. The handler rejects if the invoked action throws.
 */
...
/**
 * Formats a feature identifier into a human-readable title.
 *
 * @param featureName - Feature identifier (may contain underscores and an optional trailing `_c`)
 * @returns The formatted feature name with underscores replaced by spaces and each word capitalized (e.g., `my_feature_c` → `My Feature`)
 */
...
/**
 * Validate CopilotKit props and throw configuration errors when required credentials or prerequisites are missing.
 *
 * @param props - The CopilotKitProps object to validate.
 * @throws ConfigurationError - If neither `runtimeUrl` nor `publicApiKey` is provided.
 * @throws MissingPublicApiKeyError - If any cloud feature props (keys ending with `_c`) are present but `publicApiKey` is missing; the error message lists the affected feature names.
 */
