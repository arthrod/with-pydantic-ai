...
/**
 * Registers a frontend or catch-all Copilot action with the Copilot context and keeps it in sync while mounted.
 *
 * If the action includes a render-and-wait helper, it is adapted to a promise-based flow so the action can be executed
 * and awaited from the host; the original render logic is preserved and invoked with wait-specific props when active.
 * The hook also detects duplicate action names and emits a warning toast, updates cached render components when available,
 * and removes the registered action when the component using the hook unmounts.
 *
 * @param action - The action definition to register (frontend action or catch-all action). May include handler, render,
 *                 parameters, and optional renderAndWait / renderAndWaitForResponse for render-and-wait behavior.
 * @param dependencies - Optional dependency list that controls when the registration is re-run; if omitted, the hook
 *                       updates the action's handler and render in-place to keep closures fresh without re-registering.
 */
...
/**
 * Determines whether the given action is a frontend action.
 *
 * This function acts as a type guard that narrows the parameter to `FrontendAction<T>` when true.
 *
 * @param action - The action to check.
 * @returns `true` if the action is a frontend action, `false` otherwise.
 */
