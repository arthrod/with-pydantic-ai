...
/**
 * Create a Copilot-backed chat hook that manages chat state, message lifecycle, generation control, suggestions, and MCP server configuration.
 *
 * @param options - Optional configuration for the hook (e.g., `id`, `headers`, `initialMessages`, `makeSystemMessage`).
 * @returns An object exposing chat state and control APIs including `visibleMessages`, `appendMessage`, `setMessages`, `deleteMessage`, `reloadMessages`, `stopGeneration`, `reset`, `runChatCompletion`, `isLoading`, `mcpServers`/`setMcpServers`, `suggestions`/`setSuggestions`, `generateSuggestions`, `resetSuggestions`, `isLoadingSuggestions`, and `interrupt`.
 */
...
/**
 * Creates a ref that always holds the latest value across renders.
 *
 * The returned ref's `current` is initialized with `value` and is updated to the newest `value` whenever it changes, without causing re-renders.
 *
 * @param value - The value to keep current inside the ref
 * @returns A ref object whose `current` property always reflects the latest `value`
 */
...
/**
 * Constructs a system prompt that instructs the assistant, embedding the provided context and optional additional instructions.
 *
 * @param contextString - Textual context to include verbatim in the system prompt
 * @param additionalInstructions - Optional extra instructions to append to the prompt
 * @returns The complete system message string to use as the assistant's system prompt
 */
