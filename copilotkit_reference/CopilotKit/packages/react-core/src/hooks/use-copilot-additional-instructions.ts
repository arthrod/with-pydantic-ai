...
/**
 * Add the provided instructions to Copilot's additional instructions while the hook is active.
 *
 * When `available` is not `"disabled"`, the hook appends `instructions` to the Copilot additional
 * instructions list and removes that exact entry on cleanup or when dependencies change.
 *
 * @param options - Configuration object
 * @param options.instructions - The instruction text to add to Copilot's additional instructions
 * @param options.available - `"enabled"` to apply the instructions or `"disabled"` to skip applying them (defaults to `"enabled"`)
 * @param dependencies - Optional additional dependencies that cause the effect to re-run when they change
 */
