...
/**
 * Provide the Copilot chat hook, wiring internal behavior to Copilot context and degrading to a safe no-op implementation when a public API key is absent.
 *
 * @param options - Configuration options to control chat behavior (see `UseCopilotChatOptions`)
 * @returns The `UseCopilotChatReturn` API object: when a `publicApiKey` is configured it exposes the active functional hook result; when no `publicApiKey` is present it exposes a non-functional fallback with inert methods and empty data structures.
 */
