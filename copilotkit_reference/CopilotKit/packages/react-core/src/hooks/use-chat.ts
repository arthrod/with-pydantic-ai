...
/**
 * Creates a chat hook that manages streaming chat completions, agent/coagent state synchronization, frontend action execution, and error/guardrail handling.
 *
 * @param options - Configuration and runtime hooks for the chat flow (initial messages, message state handlers, copilot configuration, frontend actions and handlers, agent/coagent state refs and setters, abort controller, extensions, and LangGraph interrupt handlers).
 * @returns An object exposing helpers: `append(message, options)` to add a message (optionally triggering a follow-up), `reload(messageId)` to regenerate a prior assistant response, `stop()` to abort the current generation, and `runChatCompletion()` to run a completion using the current messages.
...
/**
 * Assemble the final ordered list of chat messages, using synced messages when available and inserting agent-state messages next to their original positions.
 *
 * @param syncedMessages - Messages from an authoritative sync source; when non-empty these become the base sequence instead of combining `previousMessages` and `newMessages`
 * @param previousMessages - Existing messages that precede `newMessages`
 * @param newMessages - Newly produced messages to append after `previousMessages`
 * @returns The resulting array of messages: either `syncedMessages` (with any agent-state messages from the combined history inserted immediately after the message they followed) or the concatenation of `previousMessages` and `newMessages` when no `syncedMessages` are provided
 */
...
/**
 * Executes a frontend action handler for an ActionExecutionMessage and returns a ResultMessage representing the handler outcome.
 *
 * Calls the provided function-call handler with the current message list and action args, observes abort signals while waiting for the handler, and produces a ResultMessage that encodes either the handler result or a captured error. When `isRenderAndWait` is true, updates are flushed to the UI before awaiting the handler to support render-and-wait interactions.
 *
 * @param onFunctionCall - Handler invoked with `{ messages, name, args }` to perform the frontend action.
 * @param message - The ActionExecutionMessage describing the action to execute.
 * @param chatAbortControllerRef - Mutable ref to an AbortController used to observe user aborts; an abort will cause the awaiting to resolve early.
 * @param onError - Callback invoked with any error thrown while awaiting the handler.
 * @param setMessages - State setter used to flush UI updates when `isRenderAndWait` is enabled.
 * @param getFinalMessages - Function that returns the current list of messages to pass to the handler.
 * @param isRenderAndWait - When true, flushes message updates synchronously before awaiting the handler to allow interactive render-and-wait flows.
 * @returns A ResultMessage whose `result` encodes the handler return value or an error payload, and which references the originating action execution id and name.
 */
...
/**
 * Resolve the frontend action paired with a given action execution or result message.
 *
 * @param actions - Array of frontend actions to search for a match
 * @param message - An ActionExecutionMessage or ResultMessage from which the action name is derived
 * @returns The matching `FrontendAction` whose `name` equals the message's action name and is frontend-available, or whose `pairedAction` equals that name; `undefined` if none is found
 */
