...
/**
 * Lightweight React hook that provides a headless chat API for programmatic messaging, background operations, and custom UIs.
 *
 * Accepts optional configuration (chat id, headers, initial messages, and a system message generator) and delegates behavior to the internal chat implementation without requiring a public API key.
 *
 * @param options - Optional configuration: `id` (shared chat identifier), `headers` (HTTP headers), `initialMessages` (messages to seed the chat), and `makeSystemMessage` (function to generate the system message)
 * @returns An object exposing `appendMessage`, `setSuggestions`, and `generateSuggestions` functions
 */
