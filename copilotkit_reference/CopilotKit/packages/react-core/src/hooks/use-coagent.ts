...
/**
 * Integrates a LangGraph-based CoAgent into a React component and exposes its state, lifecycle controls, and run API.
 *
 * @param options - Configuration for the agent hook. Supports either internal state management (optional `initialState`), or external state management using `state` and `setState`. May also include agent `name` and runtime `config`/`configurable` settings.
 * @returns An object containing the agent's metadata and controls: `name`, optional `nodeName` and `threadId`, `running` flag, the current `state`, a `setState` updater, and lifecycle/action methods `start`, `stop`, and `run` (accepts an optional hint function).
 */
...
/**
 * Starts an agent session for the given agent name in the Copilot context.
 *
 * @param name - The registered agent name to start
 */
...
/**
 * Stops the named agent session and marks that agent's coagent state as inactive.
 *
 * @param name - The agent's registered name to stop
 * @param context - Copilot context containing the current agent session and coagent state setters; used to clear the session and update the agent's stored state to not running/active
 */
...
/**
 * Ensures an agent session for `name`, optionally derives a hint from recent agent state, and triggers a message append or chat completion to advance the agent.
 *
 * @param name - The agent's registered name.
 * @param context - Copilot context containing session and coagent state references; may be updated to set the agent session.
 * @param messages - Recent runtime messages (GraphQL form) used to locate the most recent agent state.
 * @param appendMessage - Function to persist/emit a new Message into the conversation.
 * @param runChatCompletion - Function that runs the chat completion flow and returns produced messages.
 * @param hint - Optional function that receives `{ previousState, currentState }` and may return a Message to prepend; if it returns a message that message is appended, otherwise `runChatCompletion` is invoked.
 */
