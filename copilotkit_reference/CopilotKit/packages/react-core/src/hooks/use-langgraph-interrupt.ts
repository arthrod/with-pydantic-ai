...
/**
 * Registers and manages a LangGraph interrupt action within the Copilot context.
 *
 * Registers the provided action (assigning it a stable UUID) unless another action is already registered; if the currently registered action matches the provided action and receives a response event, this hook triggers a chat completion. If a different action is already registered and the incoming action is disabled, a warning toast is shown and the incoming action is not registered.
 *
 * @param action - A LangGraph interrupt descriptor without an `id`; the hook computes and assigns a stable `id`.
 * @param dependencies - Optional additional dependencies that control when the registration effect re-runs.
 */
