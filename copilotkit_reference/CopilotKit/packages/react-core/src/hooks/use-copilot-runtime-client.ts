...
/**
 * Create a CopilotKitError from a GraphQL error when the error maps to a known CopilotKit type.
 *
 * @param gqlError - The GraphQL error to convert into a CopilotKitError
 * @returns A CopilotKitError built from `gqlError.extensions.code` or from legacy error markers in the original error stack; `null` if no structured mapping is found
 */
