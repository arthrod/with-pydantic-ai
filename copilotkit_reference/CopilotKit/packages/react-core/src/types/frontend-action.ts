...
/**
 * Prepare frontend actions for sending to the runtime by filtering out unavailable, UI-only, paired, or wildcard actions and producing a compact metadata payload.
 *
 * @param actions - The frontend actions to process
 * @returns An array of action metadata objects, each containing `name` (action name), `description` (or empty string), `jsonSchema` (stringified parameter JSON Schema), and `available` (an `ActionInputAvailability` value)
 */
