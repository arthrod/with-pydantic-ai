  ...
  return (
    <div data-test-id="progress-steps">
      <div className="border border-slate-200 bg-slate-100/30 shadow-md rounded-lg overflow-hidden text-sm py-2">
        {logs.map((log, index) => {
          const isDone = log.type === "success" || log.type === "error";
          const isActive = log.type === "info" && index === logs.length - 1;
            ...
            >
              <div className="w-8">
                <div
                    ...
                    data-test-id={isDone ? 'progress-step-item_done' : 'progress-step-item_loading'}
                >
                  {isDone ? (
